...
import { ArrowLeft, ArrowDownToLine, ArrowUpFromLine, RefreshCw, Copy } from 'lucide-react'
        ...
        txs.map((tx) => {
          ...
          return (
            <Card key={tx.id} className="mx-5 mb-3 p-4 flex justify-between items-center">
              ...
              <span className={`font-bold text-base ${isCredit ? 'text-success' : 'text-destructive'}`}>
                {isCredit ? '+' : '-'}{Number(tx.amount).toFixed(2)} USDT
              </span>
            </Card>
          )
        })
